...
public:
  ...
  : Node("laser_gpio_driver")
  {
    ...
    sub_ =
      ...
      "/laser_command", 10,
      [this](std_msgs::msg::String::SharedPtr m) {command(m->data);});
...
private:
  ...
  {
    if (c == "ON") {
      off_at_ = now() + rclcpp::Duration::from_seconds(max_on_s_); set(true, "on");
    } else if (c == "OFF") {
      set(false, "off");
    }
  }
  ...
  {
    on_ = on;                                       /* Hardware stage: replace this branch with libgpiod set_value(). */
    std_msgs::msg::String m; m.data = state; status_->publish(m); RCLCPP_INFO(
      get_logger(), "laser %s%s", state, mock_ ? " (mock)" : "");
  }
  bool mock_{true},
  on_{false}; double max_on_s_{3.0}; rclcpp::Time off_at_;
...
{
  rclcpp::init(argc, argv); rclcpp::spin(std::make_shared<LaserGpioDriver>()); rclcpp::shutdown();
}
