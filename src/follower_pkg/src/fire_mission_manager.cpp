...
private:
  ...
  void fail(const std::string&s){state_=S::SAFE_STOP; P p;double y;if(pose(p,y))send(p,y);report("failed:"+s);} void report(const std::string&s){std_msgs::msg::String m;m.data=s;status_->publish(m);}
  double ox_,oy_,yaw0_,standoff_,tol_,laser_s_,grid_,margin_;P home_,firep_;std::vector<double>obs_;std::vector<P>path_;size_t wp_{0};std::string laser_state_;rclcpp::Time laser_start_;std::shared_ptr<tf2_ros::Buffer>tfbuf_;std::shared_ptr<tf2_ros::TransformListener>tfl_;rclcpp::Publisher<std_msgs::msg::Float32MultiArray>::SharedPtr target_;rclcpp::Publisher<std_msgs::msg::String>::SharedPtr laser_,status_;rclcpp::Subscription<std_msgs::msg::Float32MultiArray>::SharedPtr fire_;rclcpp::Subscription<std_msgs::msg::String>::SharedPtr laser_status_;rclcpp::TimerBase::SharedPtr timer_;
};
