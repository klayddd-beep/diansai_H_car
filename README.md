...
当前代码已经补齐《G题_机车通信接口约定》要求的车端通信与显示功能，并在 ROS 2 Humble 环境完成构建和本机 UDP 闭环验证。另有一个默认不启动的无人机调试视频接收预备包。场地标定、物理启动按键、触屏、实车行驶和双机联调仍需在比赛硬件上完成。

> 安全提示：激光可能伤害眼睛或引发火灾。真实激光必须使用默认断电的驱动电路、物理总断电开关及 MOSFET/光耦等隔离方案，禁止用开发板 GPIO 直接驱动激光负载。

## 本次更新加入了什么

- 新增独立预备包 `fire_video_receiver_pkg`：
  - 以 `BEST_EFFORT + KEEP_LAST(1)` 订阅域 6 的
    `/fly/fire/debug_image/compressed`，直接用 OpenCV 解码 JPEG。
  - 可选独立窗口或无界面统计模式，超过 2 秒无新帧显示/记录 `LOST`。
  - 不依赖 `cv_bridge`，不接入仪表盘，也不加入正式任务 launch。
- 新增车载相机火源微调：
...
| 无人机调试视频接收 | 预备包已实现，默认不启动 | DDS 域 6、CompressedImage、best-effort；不参与当前题目流程 |
...
无人机双域视频桥（可选、手动启动）
  -> DDS 域 6 /fly/fire/debug_image/compressed
  -> fire_video_receiver_pkg（独立窗口或无界面统计）
...
两块板的业务节点 `ROS_DOMAIN_ID` 可以不同，四条任务控制/遥测通道不依赖 DDS，
统一走 UDP。另有一条由飞机双域桥接到车端域 6 的可选调试视频，不参与任务控制，
详见接口约定 §十。坐标均为场地坐标系，原点位于巡防区左下角，x 向右、y 向上，
单位为 dm。
...
| `/fly/fire/debug_image/compressed` | `sensor_msgs/CompressedImage` | 可选无人机 JPEG 调试画面；必须 best-effort 订阅 |
...
| `src/fire_video_receiver_pkg` | 默认不启动的无人机 JPEG 调试视频接收/查看预备包 |
...
```bash
source /opt/ros/humble/setup.bash
colcon build --packages-select follower_pkg car_launch fire_video_receiver_pkg --symlink-install
```
...
入口脚本会显式设置车端 `ROS_DOMAIN_ID=6` 和 `ROS_LOCALHOST_ONLY=0`，不依赖桌面自启动是否读取 `~/.bashrc`。这样自动任务与当前车端调试终端保持在同一 DDS domain，且与无人机端的业务节点隔离；任务控制仍只通过约定的 UDP `8889/8890/8892/8893` 通信，可选视频由飞机侧双域桥单独搬到域 6。启动日志中必须出现：
...
该功能不会随正式任务启动，也没有接入仪表盘。需要联调视频时，在飞机侧启动
`fire_video_bridge` 并打开 `enable_debug_image`，车端另开终端手动运行：
...
# 不开窗口，只打印 WAITING/LIVE/LOST、帧率和码率
ros2 launch fire_video_receiver_pkg video_receiver.launch.py display:=false
```

订阅固定为 `sensor_msgs/msg/CompressedImage` 和 `BEST_EFFORT +
KEEP_LAST(1)`。超过 2 秒无有效新帧会标记 `LOST`；超时依据车端本地单调时钟，
不会拿两块板的 `header.stamp` 作差。完整接口和排查顺序见
[机车通信接口约定 §十](G题_机车通信接口约定.md#十可选调试视频无人机--消防车)。
...
- [G题_机车通信接口约定.md](G题_机车通信接口约定.md)：四条 UDP 业务通道及可选 DDS 视频的机车通信契约。
