...
| 车载遥测与航迹显示 | 已实现并模拟渲染验证 | 含可点击启动按钮；仍需手指触屏真机验证 |
...
9. `aim_tol_deg`：默认 `8°`；车辆只有在固定停车点内且车头朝向火源误差不超过此值时才开启激光。
...
13. `laser_gpio_driver`：已配置为物理 22 脚 `GPIO1_B0`（Linux GPIO 40、wPi 13），实测高电平关闭，因此使用 `active_low: true`。
14. 雷达串口、波特率和型号：修改 `src/bluesea2/src/bluesea-ros2/params/uart_lidar.yaml`。
15. 底盘串口：Orange Pi 5 Max 默认 `/dev/ttyS6`，对应 40-pin 的物理 11/13 脚。
